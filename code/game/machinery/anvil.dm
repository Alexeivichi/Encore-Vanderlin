...
/obj/machinery/anvil/proc/process_minigame_result(quality_score, mob/living/user)
	...
	if(quality_score >= MINIMUM_ANVIL_MINIGAME_SCORE) // Did you even try?
		...
		if(HAS_TRAIT(user, TRAIT_GOLERKANHFIRE) || GET_MOB_SKILL_VALUE_OLD(user, recipe_skill) < 3)// Sanity, no expert blacksmith has lower skill than 3, for if admins manually add the trait or blacksmith vampire thralls
