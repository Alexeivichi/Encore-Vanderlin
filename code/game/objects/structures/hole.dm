...
/obj/structure/closet/dirthole
	...
	/// How much passive devotion is generated by this grave. Updated by `adjust_grave_valdala_devotion()`
	var/valdala_devotion_gain = 0
	...
	if(HAS_TRAIT(L, TRAIT_GRAVEROBBER))
		...
		switch(L.patron?.type)
			if(/datum/patron/divine/valdala)
				robbing = FALSE
				message = "I perform the secret rite of exhumation, and so the Valkyrie overlooks my transgression."
			if(/datum/patron/divine/erdl) //Special notices if you're of a particular faith.
				message = "I perform the secret rite of concealment, Erdl shields me from divine gaze as I exhume this corpse for study."
			if(/datum/patron/inhumen/deceivers)
				message = "I perform the secret rite of liberation, the Gods are none the wiser as the occupant of this grave is freed."
			if(/datum/patron/inhumen/envy)
				message = "I perform the secret rite of defilement, Valdala can do nothing but watch as I undo the rites on this grave."
		to_chat(user, span_info(message))
		if(robbing)
			record_featured_stat(FEATURED_STATS_CRIMINALS, user) //You aren't a Valdalan, even though you didn't get any consequences you're still a criminal.
...
/obj/structure/closet/dirthole/proc/robbery_alert(mob/robber, var/delay = FALSE)
	...
	for (var/mob/living/player in GLOB.player_list)
		if (player.stat == DEAD || isbrain(player))
			continue
		if (is_priest_job(player.mind.assigned_role) || (is_monk_job(player.mind.assigned_role) && player.patron?.type == /datum/patron/divine/valdala) || istype(player.mind.assigned_role, /datum/job/templar) || istype(player.mind.assigned_role, /datum/job/gmtemplar) || istype(player.mind.assigned_role, /datum/job/undertaker))
...
/obj/structure/closet/dirthole/proc/adjust_grave_valdala_devotion()
	var/new_devotion = 0
	var/change = 0
	if(no_devotion)
		return
	...
	// Step 2: Compare new_devotion with valdala_devotion_gain to find change.
	change = new_devotion - valdala_devotion_gain
	valdala_devotion_gain = new_devotion
	...
	for (var/mob/living/carbon/human/player in GLOB.player_list)
		if (player.stat == DEAD || isbrain(player) || !player.cleric)
			continue
		if(player.patron?.type == /datum/patron/divine/valdala) // Person must be able to use miracles and worship Valdala
			// Grab their devotion
