...
/datum/particle_weather/rain/try_weather_act(mob/living/L)
	...
	if(can_weather_effect(L))
		...
		if(!istype(C))
			return
		var/obj/item/weapon/umbrella/held_umbrella = C.get_active_held_item() //this is absolutely frankensteined i hate it
		if(istype(held_umbrella) && held_umbrella.open)
			held_umbrella = C.get_inactive_held_item()
		if(istype(held_umbrella) || held_umbrella.open) //MORE FRANKENSTEINED CODE HELP ME
