		...
		if(istype(cast_on, /obj/structure/closet/dirthole))
			...
			if(!grave.is_consecrated)
				...
				if(grave.gravequality == 10)
					owner.visible_message(span_rose("The air gets colder as [owner] consecrates [cast_on], woe betide any graverobber."), span_rose("Valdala's gaze turns over to [cast_on] as I consecrate it. Any who would rob this grave will feel the Valkyrie's full wrath!"))
					grave.grow_lily()
			grave.adjust_grave_valdala_devotion()
			grave.stasis()
			return
...
/datum/action/cooldown/spell/burial_rites/proc/generate_inscription(obj/structure/closet/dirthole/grave, obj/item/gravedecor/headstone/headstone)
	...
	for(var/final_words in their_final_words)
		headstone.inscription += SPAN_GOD_VALDALA("<br>[final_words]")
