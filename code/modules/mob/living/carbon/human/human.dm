			...
			if(CPR_CHEST)
				...
				if(diceroll <= DICE_CRIT_FAILURE) // can't even break ribs correctly
					...
					if(HAS_TRAIT(target, TRAIT_VALDALA_CURSE))
						to_chat(src, span_warning("Valdala holds tight to this one."))
						return FALSE
...
/mob/living/carbon/human/vv_do_topic(list/href_list)
	...
	if(href_list[VV_HK_CORONATE])
		...
		priority_announce("The Aspects have named [coronated.real_name] the inheritor of [SSmapping.config.map_name]!", \
